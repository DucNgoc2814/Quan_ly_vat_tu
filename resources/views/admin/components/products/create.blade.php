                ...
                <div class="card-body">
                    <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="mt-3">
                                <button class="btn btn-success text" id="btn-submit"
                                    onclick="validateVariantQuantities(event)">Thêm mới</button>
                            </div>
                        </div>
                    </form>
                </div>
...
`;
    ...
    `;
        ...
        function validateVariantQuantities(event) {
            const quantityInputs = document.querySelectorAll('input[name$="[stock]"]'); // Lấy tất cả các ô nhập số lượng
            let isValid = true; // Biến kiểm tra trạng thái hợp lệ

            quantityInputs.forEach(input => {
                const value = input.value.trim(); // Lấy giá trị nhập
                const parentDiv = input.closest('.mb-2'); // Container gần nhất
                ...
                if (value === '' || isNaN(value) || Number(value) <= 0) {
                    ...
                    input.classList.add('is-invalid'); // Thêm class CSS lỗi

                    // Tạo thông báo lỗi
                    const errorDiv = document.createElement('span');
                    errorDiv.className = 'text-danger';
                    errorDiv.style.fontSize = '12px';
                    errorDiv.textContent = 'Vui lòng nhập số lượng hợp lệ (lớn hơn 0).';

                    // Chèn thông báo lỗi ngay dưới ô input
                    input.insertAdjacentElement('afterend', errorDiv);
                } else {
                    input.classList.remove('is-invalid'); // Xóa class lỗi nếu hợp lệ
                }
            });
        ...
        console.log(input); // Kiểm tra ô nhập đang được xử lý
        console.log('123');
    </script>
@endsection
