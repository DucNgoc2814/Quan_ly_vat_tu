...
<script>
    function changeSelect(event) {
        ...
        const data = @json($topProductsData);
        
        renderTopProducts(data[period]);
    }
    ...
    function renderTopProducts(data) {
        ...
        if (!data || !data.length) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="5" class="text-center">Không có dữ liệu cho khoảng thời gian này</td>
                </tr>`;
            return;
        }
        ...
        const html = data.map((item, index) => {
            return `
                <tr>
                    ...
                    <td class="text-end">${item.quantity_sold || 0}</td>
                    <td class="text-end">${formatCurrency(item.average_price || 0)}</td>
                </tr>
            `;
        }).join('');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (periodSelect) {
            changeSelect({ target: periodSelect });
        }
    });
</script>
@endpush
